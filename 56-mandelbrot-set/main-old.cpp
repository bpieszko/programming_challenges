...
void f_thread2(sf::RenderWindow & ren, double & a, double & z, double & r, bool & ok, std::stack<std::tuple<double, double, double>> & zoom_coords) {
	while (ren.isOpen()) {
		int iter_max = 500;
		const char *fname = "mandelbrot.png";
		bool smooth_color = true;

		a = std::get<0>(zoom_coords.top());
		z = std::get<1>(zoom_coords.top());
		r = std::get<2>(zoom_coords.top());
...
int main () {
	...
	zoom_coords.push({a, z, r});

	std::thread second(f_thread2, std::ref(render), std::ref(a), std::ref(z), std::ref(r), std::ref(ok), std::ref(zoom_coords));
	second.detach();
	...
	while (render.isOpen()) {
		...
		text.setString(std::to_string(4.0/std::get<2>(zoom_coords.top())));
